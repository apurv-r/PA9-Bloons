...
std::string mapGen::generateRandomString(int length) {
    std::string result = "";
...
void mapGen::run(sf::RenderWindow &window)
{
    sf::Text helperText;
...
std::vector<sf::Vector2f> mapGen::loadMapRandom(std::vector<std::string> files)
{
    if (files.size() > 0)
    {
        ...
        std::ifstream infile(files[index]);
        ...
        {
            std::cout << "Error loading in file " << files[index] << std::endl;
            return std::vector<sf::Vector2f>();
        }
    }
    else
    {
        //Effectivly a null return
        //Could have tried a exception catcher but I still need a return object.
        return std::vector<sf::Vector2f>();
    }
}

std::vector<sf::Vector2f> mapGen::loadMapSpecific(std::string files)
{
    std::vector<sf::Vector2f> coords;
